class Api::V1::Auth::ChangePasswordController < Api::V1::AuthorizeController
  validate_params on: :update, require: User::CHANGE_PASSWORD_PARAMS + [:device_id]

  def update
    if current_user.update_with_password change_password_params
      Authenticates::RevokeAccessService.new(user: current_user, device_token: params[:device_id]).perform
